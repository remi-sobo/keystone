...
- **2026-07-08, Ring 4 (Deliverables and library), built.** Migration 0006 (applied live): deliverables (kind file or link with a payload constraint, both scope columns, both-dimension read, practice-only writes), resources (the practice-wide catalog and the documented no-client_id case: readable by every client member of the practice, written by consultants only), and two private storage buckets whose object paths carry the scope ids as folders so path-scoped SELECT policies on storage.objects enforce the table walls; private.try_uuid fails closed on malformed paths, and neither bucket carries any write policy (uploads ride signed upload URLs minted server-side after the checks, deletes ride the service role). One FLAGGED schema addition: session_prep_resources, the join behind "prep resources surfaced above upcoming sessions" (spec 6.4 names the surface, the 5.1 schema block has no mechanism for it; this is the minimal one, with both-dimension read so a sibling client never sees what another engagement was handed). Practice ships from the engagement page (direct-to-storage upload, honest failure states, remove deletes row and object together); the client timeline at /deliverables unrolls down the brass hairline and downloads run on the client's OWN session so the storage policy serves the bytes; authoring at /library/authoring passes every body through the voice gate; client read at /library and /library/[id]. The scratch-Postgres stub gained a minimal storage schema so the live matrix now exercises the storage policies too (cross-client and cross-practice object reads, the malformed-path case). 78 static gate assertions green; no new env vars.
- **2026-07-08, Ring 5 (Messages), built.** Migration 0007 (applied live): message_threads (one per engagement; the unique constraint states the v1 model honestly) and messages (author-stamped; author_side denormalized so read states and the unanswered queue never join memberships). The insert policy demands self-authorship, the correct side of the wall, and the caller's own scope (client writes ride message.write through the permission authority); bodies are immutable to every session (UPDATE revoked, then re-granted on read_at alone) and nothing deletes: correspondence is a record. Client sends at /messages, practice owners get a deep-linked email whose targets come from keystone_message_notify_targets (SECURITY DEFINER, membership-checked, owner emails only, revoked from anon: the pure-RLS client surface cannot and must not read practice_members); the practice replies from the engagement mission control (#messages) and the reply emails the thread's client participants (all client members when the practice speaks first) and marks the client's messages read. The /today card is live: threads whose last word is the client's, with age in days. Both paths are rate limited and degrade honestly (a saved message with a failed email says so, per the section 9 silent-email-failure risk). Two v1 decisions logged for review: notification targets are practice OWNERS (the spec's "Remi gets an email"), and reply emails go to client members who have spoken in the thread, falling back to the whole client roster. Live matrix extended (forged authorship, wrong wall, cross-client thread, body rewrite, RPC disclosure all refused); 86 static gate assertions green. The live end-to-end email run waits on RESEND_API_KEY (setup checklist).
- **2026-07-09, Ring 6 (Weekly digest), built.** Migration 0008 (applied live): digests, the record of approved digests, practice-only read and zero session write policies (the approve action is the single path in, service role after the membership check), unique per engagement and week. lib/digest.ts is pure (e2e/digest-engine.spec.ts): fact lines from the week's REAL rows, hasDigestContent refuses an empty week before any model call (upcoming sessions alone do not make a week), a forced submit tool with the data-not-instructions guard, Zod re-validation. The cron at /api/digest (vercel.json schedule 0 22 * * 5, the CONFIRM 6 proposal of Friday 3pm Pacific; 22:00 UTC is 3pm PDT and 2pm PST, revisit at the gate) fails closed on a missing CRON_SECRET, drafts per active engagement on the digest tier through the one chokepoint (spend guard, ledger, refusal fallback), voice-sweeps, and writes ONE inert ai_proposals row; it never touches a live table and never sends email. The /today approval queue is live: read the draft, approve (branded email per client member, replyTo the consultant; the record marks 'sent' only when every send succeeded, 'approved' with an honest state otherwise) or dismiss. Live matrix extended (digests invisible to every client session, unwritable by every session); 99 static gate assertions green. The end-to-end run waits on ANTHROPIC_API_KEY, RESEND_API_KEY, and CRON_SECRET, all already on the setup checklist.
