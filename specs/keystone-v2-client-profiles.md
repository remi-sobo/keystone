...
**Status:** BUILT 2026-07-11 (all five gates approved as recommended). Migration 0034 (the practice-only `client_profiles` table), `/clients/[id]` the profile page, `/clients` upgraded with the health phrase per row, `src/lib/healthInputs.ts` the shared health assembly (the /engagements list refactored onto it so momentum reads the same on both surfaces), the isolation matrix extended, `e2e/client-profile.spec.ts`. Code-reviewed: the review caught the first cut putting the facts as columns on the client-readable `clients` row (a client member could read the private relationship note straight from the REST endpoint, since RLS is row-level and clients_read admits the client's own member); fixed by moving the facts to a practice-only `client_profiles` table read by is_practice_member alone, the 1B (engagement_drafts) precedent, so the client session sees nothing here by construction. The live matrix now proves the client's own member reads ZERO profile rows, a consultant reads but cannot write, and both classic walls hold. 290 gate assertions green; the live matrix passes on scratch Postgres 16; typecheck, lint, build clean. The fee shows on the profile per CP-2, logged below as the gate-9 amendment. 390px live run owed with the other rings (blocked on the auth allow-list like every surface).
**Date:** 2026-07-11
...
- **A practice-only `client_profiles` table (gate CP-3).** Org-level facts the schema lacks: `relationship_note` (text, the practice's one-line why-this-client), `primary_contact_member_id` (fk into `client_members`, on delete set null), `website` (text), `relationship_started_on` (date, distinct from the client row's `created_at`, because a client may predate their Keystone record), on a table carrying both scope ids (`client_id` unique, `practice_id`), one row per client. **Not columns on `clients`.** The clients row is client-readable (`clients_read` = `is_practice_member OR is_member_of_client`), and RLS is row-level, so a private practice note added there would be readable by that client's own member straight from the REST endpoint even with no client-surface code selecting it, and the pure-RLS client surface must be walled by RLS, not by app-code omission. So the facts live on their own table whose read policy is `is_practice_member` alone (a client session matches nothing), the exact reasoning 1B used to reject a draft flag on the client-visible `engagements` table in favor of a practice-only `engagement_drafts` table. Writes are owner-only (`keystone_can(practice_id, null, 'practice.manage')`); reads admit any practice member. The isolation test in the same PR proves the client's own member reads zero, a consultant reads but cannot write, and both classic walls hold.
...
- **New scoped table (`client_profiles`):** carries `practice_id` and `client_id`, RLS on, membership read policy (`is_practice_member`, practice-only so the client never sees it), owner-only write; the same PR ships the cross-practice AND cross-client isolation test, plus the practice-only assertion (the client's own member reads zero) that is the whole reason this is a separate table.
...
| CP-3 | Add the four org-level facts (relationship note, primary contact, website, relationship-started date) with the isolation test in the same PR, or compose from existing data only? | Add the four fields, on a PRACTICE-ONLY `client_profiles` table (not columns on the client-readable `clients` row, which would leak the private note to the client member under row-level RLS). The 1B/engagement_drafts precedent; the client session sees nothing here by construction |
