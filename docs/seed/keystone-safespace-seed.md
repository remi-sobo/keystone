...
17. **Weekly time expectation set: five to eight hours per coachee per week,** covering the session, the homework, and implementation time, like training; heavier in month one. (Remi, Jul 11.)
...
- **From Susan's agreement review (Jul 11):** the weekly time range is decided at five to eight hours per coachee per week (decision 17). The agreement redline lives at `docs/seed/safespace-agreement-v2-draft.md`; the installment dates and the hub security description are confirmed (Remi, Jul 11), and the one remaining bracket is the board approval date in section 12, open until the disinterested board members vote. Charter v2 publishes in-app from the updated §3 body; per gate 2A-2 that withdraws v1's pending sign-off and re-requests against the new version. The live library's ledger resource needs the same §8 additions via /library/authoring. The export commitment makes V2 5B (engagement export and portability) a contractual obligation: with the 30-day termination clause, handover must be possible from early in the engagement, so 5B should be specced next, not at closeout.
